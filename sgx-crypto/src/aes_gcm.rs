...
impl AESGCM {
    ...
    pub fn new_with_key(key: &[u8], key_len:u32) -> super::Result<Self> {
        ...
        let mut inner = Cipher::setup(CipherId::Aes, CipherMode::GCM, (key_len * 8) as u32).expect("Cipher::setup error");
    ...
    pub fn encrypt(
        ...
        let ad = [0u8, 0, 0, 0];
        let len = self.inner.encrypt_auth(&ad, plain_text, cipher_text, tag).expect("Cipher::encrypt_auth");
        Ok(len)
    }
    ...
    pub fn decrypt(
        ...
        let ad = [0u8, 0, 0, 0];
        Ok(self.inner.decrypt_auth(&ad, cipher_text, plain_text, tag)?)
    }
}
